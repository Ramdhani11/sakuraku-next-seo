...
async function getArticles() {
  try {
    const res = await fetch("https://api.sakuraku.id/api/v1/articles", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({}), // isi body sesuai kebutuhan API
      next: { revalidate: 3600 },
    });
    const data = await res.json();
    // Filter hanya artikel yang belum punya sentences (sama seperti logika di Vite)
    const all = data?.value ?? [];
    return all.filter((a) => !a.sentences || a.sentences.length < 1);
  } catch {
    return [];
  }
}
